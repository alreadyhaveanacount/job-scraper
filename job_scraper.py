...
def to_monthly_salary(contract, pay_rate, average_pay):
    if pay_rate == "month": return average_pay

    if pay_rate == "year": return round(average_pay / 12, 2)
    ...
    return average_pay * hours_per_week * WEEKS_PER_MONTH
...
def load_page(buffer: List[Dict], specifics: Dict[str, Union[str, List[str]]]={}) -> None:
    ...
    for job in jobs:
        ...
        if min_salary is not None and max_salary is not None:
            job_data["Monthly Salary(average)"] = to_monthly_salary(job_data["Contract"], pay_rate[1:], (min_salary+max_salary)/2)
